...
BADGE_DEFS = [
    ...
    {"id": "ati_100", "name": "Maitre", "desc": "100 ATIs traites", "icon": "\U0001f451", "category": "productivite"},
    {
        "id": "sla_perfect",
        "name": "Ponctuel",
        "desc": "10 ATIs traites dans les delais SLA",
        "icon": "\u23f1\ufe0f",
        "category": "qualite",
    },
    {
        "id": "zero_reject",
        "name": "Sans faute",
        "desc": "10 decisions consecutives sans rejet",
        "icon": "\u2728",
        "category": "qualite",
    },
    {
        "id": "inspector_5",
        "name": "Inspecteur actif",
        "desc": "5 inspections realisees",
        "icon": "\U0001f50d",
        "category": "terrain",
    },
    {
        "id": "inspector_20",
        "name": "Inspecteur senior",
        "desc": "20 inspections realisees",
        "icon": "\U0001f396\ufe0f",
        "category": "terrain",
    },
    {
        "id": "multi_sector",
        "name": "Polyvalent",
        "desc": "ATIs dans 3+ secteurs differents",
        "icon": "\U0001f30d",
        "category": "diversite",
    },
    {
    ...
    },
    {
        "id": "streak_7",
        "name": "Regulier",
        "desc": "Actif 7 jours consecutifs",
        "icon": "\U0001f525",
        "category": "engagement",
    },
]
...
def compute_badges(db: Session, username: str) -> list[dict]:
    ...
    # Count inspections
    inspections = (
        db.execute(select(InspectionConformiteORM).where(InspectionConformiteORM.inspecteur_username == username))
        .scalars()
        .all()
    )

    # Count audit events
    events = db.execute(select(AuditEventORM).where(AuditEventORM.actor == username)).scalars().all()
    ...
    on_time = [
        a
        for a in decided
        if a.date_decision and (a.date_decision.date() - a.date_soumission.date()).days <= a.sla_jours
    ]
    if len(on_time) >= 10:
