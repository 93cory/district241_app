...
def generate_executive_report(db: Session) -> bytes:
    ...
    month_approvals = sum(
        1 for a in all_atis if a.statut == "approuve" and a.date_decision and _as_utc(a.date_decision) >= month_ago
    )

    overdue = sum(
        1 for a in all_atis if a.statut not in _TERMINAL and (now.date() - a.date_soumission.date()).days > a.sla_jours
    )
    ...
    conformes = sum(
        ...
        if i.date_inspection and _as_utc(i.date_inspection) >= month_ago and i.statut_conformite == "conforme"
    )
    ...
    header_style = ParagraphStyle(
        "header", parent=styles["Normal"], alignment=TA_CENTER, fontSize=9, textColor=colors.gray
    )
    ministry_style = ParagraphStyle(
        "ministry", parent=styles["Normal"], alignment=TA_CENTER, fontSize=10, textColor=BLEU, fontName="Helvetica-Bold"
    )
    title_style = ParagraphStyle("report_title", parent=styles["Title"], textColor=BLEU, fontSize=18, spaceAfter=4)
    section_style = ParagraphStyle(
        "section", parent=styles["Heading2"], textColor=BLEU, fontSize=13, spaceAfter=6, spaceBefore=12
    )
    ...
    story.append(
        Paragraph(
            ...
            ParagraphStyle("motto", parent=header_style, fontSize=8, fontName="Helvetica-Oblique"),
        )
    )
    ...
    story.append(
        Paragraph(
            ...
            ParagraphStyle("pnpi", parent=ministry_style, fontSize=9),
        )
    )
    story.append(Spacer(1, 0.3 * cm))
    ...
    story.append(
        Paragraph(
            f"Semaine du {week_ago.strftime('%d/%m/%Y')} au {now.strftime('%d/%m/%Y')}",
            ParagraphStyle("date", parent=body_style, textColor=colors.gray, fontSize=9),
        )
    )
    story.append(
        Paragraph(
            "CONFIDENTIEL · Document officiel PNPI",
            ParagraphStyle("conf", parent=body_style, textColor=ROUGE, fontSize=8, fontName="Helvetica-Bold"),
        )
    )
    ...
    story.append(
        Paragraph(
            f"<font size='28' color='{nti_color}'><b>{nti}</b></font><font size='12'> / 100</font>",
            ParagraphStyle("nti", parent=body_style, alignment=TA_CENTER, spaceAfter=12),
        )
    )
    ...
    pipeline_data = [
        ...
        [
            str(pipeline.get("soumis", 0)),
            str(pipeline.get("en_instruction", 0)),
            str(pipeline.get("en_validation", 0)),
            str(pipeline.get("approuve", 0)),
            str(pipeline.get("rejete", 0)),
            str(pipeline.get("expire", 0)),
        ],
    ]
    ...
    story.append(
        Paragraph(f"Document genere automatiquement par la PNPI · {now.strftime('%d/%m/%Y %H:%M')} UTC", footer_style)
    )
