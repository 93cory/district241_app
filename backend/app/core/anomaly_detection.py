...
def detect_anomalies(db: Session) -> list[dict]:
    ...
    if prev_7d and len(last_7d) > len(prev_7d) * 2:
        alerts.append({
            ...
            "detail": f"{len(last_7d)} soumissions cette semaine vs {len(prev_7d)} la semaine precedente ({len(last_7d)/len(prev_7d):.1f}x).",
            "recommendation": "Verifier si un evenement externe explique ce pic. Anticiper la charge de travail.",
        })

    # 2. Approval rate drop
    last_30d = [a for a in all_atis if a.statut in ("approuve", "rejete") and a.date_decision and (now - a.date_decision).days <= 30]
    prev_30d = [a for a in all_atis if a.statut in ("approuve", "rejete") and a.date_decision and 30 < (now - a.date_decision).days <= 60]
    if len(last_30d) >= 5 and len(prev_30d) >= 5:
        recent_rate = sum(1 for a in last_30d if a.statut == "approuve") / len(last_30d) * 100
        prev_rate = sum(1 for a in prev_30d if a.statut == "approuve") / len(prev_30d) * 100
    ...
    if last_7d:
        for sect, count in sector_counts.items():
            if count / len(last_7d) > 0.6 and len(last_7d) >= 5:
                alerts.append({
                    "type": "sector_concentration",
                    "severity": "info",
                    "title": f"Concentration sectorielle: {sect}",
                    "detail": f"{count}/{len(last_7d)} soumissions cette semaine dans le secteur {sect} ({count/len(last_7d)*100:.0f}%).",
    ...
    recent_atis = [a for a in all_atis if a.statut == "rejete" and a.date_decision and (now - a.date_decision).days <= 90]
    for a in recent_atis:
        if a.operateur:
            op_rejections[a.operateur.raison_sociale] += 1
    ...
    # 6. SLA breach wave
    overdue = [a for a in pending if (now.date() - a.date_soumission.date()).days > a.sla_jours]
    if len(overdue) >= 5:
        alerts.append({
            "type": "sla_wave",
            "severity": "critical",
