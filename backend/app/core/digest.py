...
def generate_daily_digest(db: Session, username: str, roles: list[str]) -> dict:
    ...
    if "instructeur" in roles:
        pending = [
            a
            for a in all_atis
            if a.statut not in ("approuve", "rejete", "expire") and getattr(a, "instructeur_username", None) == username
        ]
    ...
    # Recent inspections
    recent_insp = (
        db.execute(select(InspectionConformiteORM).where(InspectionConformiteORM.date_inspection >= yesterday))
        .scalars()
        .all()
    )

    # Audit events count
    events_count = db.execute(select(AuditEventORM).where(AuditEventORM.timestamp >= yesterday)).scalars().all()
        ...
        sections.append(
            {
                ...
                "icon": "\U0001f4cb",
                "items": [
                    f"{a.numero_ati} · {a.operateur.raison_sociale if a.operateur else '?'}" for a in new_atis[:5]
                ],
            }
        )
        ...
        sections.append(
            {
                ...
                "icon": "\U0001f6a8",
                "items": [
                    f"{a.numero_ati} · {(now.date() - a.date_soumission.date()).days}j / {a.sla_jours}j SLA"
                    for a in overdue[:5]
                ],
            }
        )
