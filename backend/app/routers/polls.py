...
async def get_active_polls(current_user: User = Depends(get_current_user), db: Session = Depends(get_db)):
    polls = (
        db.execute(select(PollORM).where(PollORM.is_active.is_(True)).order_by(PollORM.created_at.desc()))
        .scalars()
        .all()
    )
    result = []
...
async def create_poll(
    data: dict, current_user: User = Depends(require_roles(Role.admin, Role.directeur)), db: Session = Depends(get_db)
):
    ...
    poll = PollORM(
        id=str(uuid.uuid4()), question=question, options=json.dumps(options), created_by=current_user.username
    )
    db.add(poll)
    db.commit()
