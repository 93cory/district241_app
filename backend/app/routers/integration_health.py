...
async def integration_health_status(
    ...
    if s3_endpoint:
        try:
            ...
            checks.append({"name": "MinIO / S3", "type": "storage", "status": "down", "latency_ms": None, "detail": "Connection failed"})
    else:
        checks.append({"name": "MinIO / S3", "type": "storage", "status": "not_configured", "latency_ms": None, "detail": "PNPI_S3_ENDPOINT non defini"})

    # 3. SMTP
    smtp_host = os.environ.get("PNPI_SMTP_HOST", "")
    if smtp_host:
        checks.append({"name": "SMTP Email", "type": "email", "status": "configured", "latency_ms": None, "detail": f"Host: {smtp_host}"})
    else:
        checks.append({"name": "SMTP Email", "type": "email", "status": "not_configured", "latency_ms": None, "detail": "PNPI_SMTP_HOST non defini"})

    # 4. Integration API Keys
    for system_name, env_var in [("DGDI (Douanes)", "PNPI_DGDI_KEY"), ("DGI (Impots)", "PNPI_DGI_KEY"), ("MTEPS (Emploi)", "PNPI_MTEPS_KEY")]:
    ...
    # 5. Prometheus
    checks.append({"name": "Prometheus", "type": "monitoring", "status": "operational", "latency_ms": None, "detail": "Metrics endpoint /metrics actif"})
