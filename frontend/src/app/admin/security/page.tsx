...
export default function SecurityPage() {
  ...
  useEffect(() => {
    Promise.all([
      ...
      fetch("/api/admin/audit-logs?action=auth.login_failed&limit=20").then(r => r.ok ? r.json() : { events: [] }),
      fetch("/api/admin/audit-logs?action=auth.login&limit=20").then(r => r.ok ? r.json() : { events: [] }),
    ...
    <div style={{ padding: "24px 32px", maxWidth: 1000, margin: "0 auto" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(4, 1fr)", gap: 12, marginBottom: 20 }}>
        {[
          ...
          { label: "Taux erreur", value: `${data.usage.error_rate || 0}%`, color: data.usage.error_rate > 5 ? "#b42318" : "#006233" },
          { label: "Echecs connexion", value: failedCount, color: failedCount > 5 ? "#b42318" : "#d97706" },
        ].map(k => (
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
        ...
        <div className="chart-card" style={{ padding: 16 }}>
          <h3 style={{ fontSize: 14, fontWeight: 700, margin: "0 0 10px", color: "#b42318" }}>Tentatives echouees</h3>
          {data.failed_logins.length === 0 ? (
            <p style={{ fontSize: 12, color: "var(--text-soft)" }}>Aucune tentative echouee recente.</p>
          ) : (
            <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
              {data.failed_logins.map((e: any) => (
                <div key={e.id} style={{ display: "flex", justifyContent: "space-between", fontSize: 12, padding: "4px 0", borderBottom: "1px solid var(--line, #eee)" }}>
                  <span style={{ fontWeight: 600 }}>{e.actor || "inconnu"}</span>
                  <span style={{ color: "var(--text-soft, #9ca3af)", fontSize: 10 }}>{new Date(e.created_at).toLocaleString("fr-FR")}</span>
                </div>
              ))}
            </div>
          )}
        ...
        {/* Recent logins */}
        <div className="chart-card" style={{ padding: 16 }}>
          <h3 style={{ fontSize: 14, fontWeight: 700, margin: "0 0 10px", color: "#006233" }}>Connexions recentes</h3>
