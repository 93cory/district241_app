      ...
      <div className="chart-card">
        <div className="pnpi-page-head">
          <div>
            ...
            <p className="pnpi-page-sub">
              ...
              <strong>{data?.s3_enabled ? "S3/MinIO + local" : "local"}</strong>{" "}
              &middot; repertoire <span className="pnpi-mono">{data?.dir ?? "—"}</span>
            </p>
          </div>
        ...
        {!data?.backups?.length ? (
          <div className="pnpi-empty">
            ...
            <div>Aucune sauvegarde a ce jour. Cliquez sur &laquo; Creer une sauvegarde &raquo;.</div>
          </div>
        ) : (
          <div className="table-scroll">
            <table className="annex-table">
              ...
              <tbody>
                {data.backups.map((b) => (
                  <tr key={b.name}>
                    <td><span className="pnpi-mono">{b.name}</span></td>
