...
interface KpiData {
  atis_total: number; atis_approuves: number; atis_en_cours: number;
  operateurs_actifs: number; inspections_total: number; sla_overdue: number;
}
...
export default function KioskPage() {
  ...
  useEffect(() => {
    ...
    return () => { clearInterval(dataInterval); clearInterval(clockInterval); clearInterval(slideInterval); };
  }, []);

  const slides = [
    // Slide 1: KPIs
    <div key="kpis" style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 32, padding: "0 60px" }}>
      {[
        ...
        { label: "SLA Depasses", value: kpis?.sla_overdue || 0, color: kpis?.sla_overdue ? "#f87171" : "#4ade80" },
      ].map(k => (
        <div key={k.label} style={{ textAlign: "center" }}>
          <div style={{ fontSize: 72, fontWeight: 800, color: k.color, lineHeight: 1 }}>{k.value}</div>
    ...
    <div key="bignums" style={{ textAlign: "center" }}>
      <div style={{ fontSize: 140, fontWeight: 800, lineHeight: 1 }}>{kpis?.atis_approuves || 0}</div>
