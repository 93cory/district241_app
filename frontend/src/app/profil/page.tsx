...
export default async function ProfilPage() {
  ...
  const isOperateurOrInstructeur = (profile.roles ?? []).some((r) =>
    ["operateur", "instructeur", "directeur", "admin"].includes(r),
  );

  const [myATIs, myInspections] = await Promise.all([
    isOperateurOrInstructeur ? fetchPNPIATIs({ limit: 10 }).catch(() => []) : Promise.resolve([]),
    isInspecteur
      ? fetchPNPIInspections({ inspecteur_username: profile.username, limit: 10 }).catch(() => [])
      : Promise.resolve([]),
  ]);

  const createdATIs = myATIs.filter(
    (a) =>
      a.instructeur_username === profile.username || (profile.roles ?? []).includes("operateur"),
  );
  ...
  return (
    <section className="section">
      <div style={{ marginBottom: "0.75rem", fontSize: "0.875rem" }}>
        <Link
          href={(profile.roles ?? []).includes("inspecteur") ? "/inspecteur" : "/pnpi"}
      ...
      <div style={{ display: "flex", gap: "1.25rem", flexWrap: "wrap" }}>
          ...
          <div className="chart-card" style={{ padding: "1.5rem" }}>
            ...
            >
              ...
              <div>
                ...
                <div style={{ fontFamily: "monospace", fontSize: "0.82rem", color: "#6b7280" }}>
                  @{profile.username}
                </div>
              </div>
            </div>
              ...
              {isOperateurOrInstructeur && (
                ...
                >
                  <div style={{ fontSize: "1.5rem", fontWeight: 800, color: "#10b981" }}>
                    {createdATIs.filter((a) => a.statut === "approuve").length}
                  </div>
              ...
              {isOperateurOrInstructeur && (
                ...
                >
                  <div style={{ fontSize: "1.5rem", fontWeight: 800, color: "#d97706" }}>
                    {createdATIs.filter((a) => a.is_overdue).length}
                  </div>
            ...
            >
              ...
              <div style={{ display: "flex", flexDirection: "column", gap: "0.5rem" }}>
                {(profile.roles ?? []).some((r) =>
                  ["admin", "ministre", "directeur", "instructeur", "inspecteur"].includes(r),
                ) && (
                  <Link
                    href="/pnpi"
                    style={{
        ...
        {/* Activite recente */}
        <div
          style={{ flex: "2 1 380px", display: "flex", flexDirection: "column", gap: "1.25rem" }}
        >
          {/* ATIs recents */}
          {isOperateurOrInstructeur && (
            <div className="chart-card" style={{ padding: "1.25rem" }}>
              <h3 style={{ margin: "0 0 1rem", color: "#003F8F", fontSize: "0.95rem" }}>
                Mes dossiers ATI recents
              </h3>
              {createdATIs.length === 0 ? (
                <p style={{ color: "#6b7280", margin: 0, fontSize: "0.82rem" }}>
                  Aucun dossier ATI associe.
                </p>
              ) : (
                      ...
                      >
                        ...
                        <span
                          style={{
                            ...
                            borderRadius: "999px",
                            background: `${STATUT_COLORS[a.statut] ?? "#6b7280"}18`,
                            color: STATUT_COLORS[a.statut] ?? "#6b7280",
                            fontWeight: 700,
                            fontSize: "0.7rem",
                            whiteSpace: "nowrap",
                            flexShrink: 0,
                ...
                {myInspections.slice(0, 5).map((insp) => (
                  ...
                  >
                    ...
                    >
                      ...
                      <span
                        style={{
                          padding: "0.15rem 0.5rem",
                          borderRadius: "999px",
                          background: `${CONF_COLORS[insp.statut_conformite] ?? "#6b7280"}18`,
                          color: CONF_COLORS[insp.statut_conformite] ?? "#6b7280",
                          fontWeight: 700,
                          fontSize: "0.7rem",
                        }}
                      >
                    ...
                    <div style={{ fontSize: "0.78rem", color: "#374151" }}>
                      {insp.observations.slice(0, 80)}
                      {insp.observations.length > 80 ? "..." : ""}
                    </div>
