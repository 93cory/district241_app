...
export default async function VerifyOperateurPage({ params }: Props) {
  ...
  try {
    ...
    const res = await fetch(
      `${backendUrl}/integration/verify-operateur/${encodeURIComponent(nif)}`,
      {
        headers: {
          "X-Api-Key": process.env.PNPI_INTERNAL_KEY || "internal",
          "X-System-Id": "dgdi",
        },
        cache: "no-store",
      },
      ...
      >
        <div style={{ textAlign: "center", marginBottom: 20 }}>
          ...
          <h1 style={{ fontSize: 18, fontWeight: 800, margin: "10px 0 2px", color: "#051B36" }}>
            Carte d'identite industrielle
          </h1>
          <p style={{ fontSize: 12, color: "#526175", margin: 0 }}>
            Verification d'operateur · PNPI
          </p>
        </div>
          ...
          data?.found && (
            <>
              <div style={{ textAlign: "center", marginBottom: 16 }}>
                <div
                  style={{
                    ...
                    borderRadius: "50%",
                    margin: "0 auto 8px",
                    background: data.is_active ? "#dcfce7" : "#fef2f2",
                    display: "flex",
                    alignItems: "center",
                    justifyContent: "center",
                    border: `3px solid ${data.is_active ? "#006233" : "#b42318"}`,
                  }}
                >
                  <span style={{ fontSize: 32 }}>{data.is_active ? "\u2705" : "\u274C"}</span>
                </div>
                <div style={{ fontSize: 20, fontWeight: 800, color: "#051B36" }}>
                  {data.raison_sociale}
                </div>
                <div style={{ fontSize: 13, color: "#526175" }}>
                  {data.is_active ? "Operateur actif et verifie" : "Operateur inactif"}
                </div>
              </div>
              ...
              {data.active_atis && data.active_atis.length > 0 && (
                <div>
                  <div style={{ fontSize: 12, fontWeight: 700, marginBottom: 6, color: "#006233" }}>
                    Agrements actifs :
                  </div>
                  {data.active_atis.map((ati: any) => (
                    ...
                    >
                      ...
                      <span style={{ color: "#526175", textTransform: "capitalize" }}>
                        {ati.secteur}
                      </span>
                    </div>
                  ))}
