...
export default async function VerifyProductPage({ searchParams }: Props) {
  ...
  if (ati) {
    try {
      ...
      const res = await fetch(`${backendUrl}/pnpi/ati/verify/${encodeURIComponent(ati)}`, { cache: "no-store" });
  ...
  return (
    <div style={{
      ...
      <div style={{
        ...
        {error && (
          <div style={{ padding: 14, borderRadius: 12, background: "#fef2f2", color: "#b42318", textAlign: "center", fontSize: 13 }}>
            {error}
          </div>
        )}
        ...
        {data && (
          <>
            <div style={{
              textAlign: "center", padding: "14px 16px", borderRadius: 14, marginBottom: 16,
              ...
              {valid && <div style={{ fontSize: 11, color: "#006233", marginTop: 2 }}>Ce produit est fabrique par un operateur agree.</div>}
            </div>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {[
                ...
                <div key={label as string} style={{ display: "flex", justifyContent: "space-between", padding: "5px 0", borderBottom: "1px solid #f0f0f0", fontSize: 13 }}>
