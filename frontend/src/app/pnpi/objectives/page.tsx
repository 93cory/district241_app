...
const DEFAULT_OBJECTIVES: Objective[] = [
  { id: "obj-dossiers", label: "Dossiers traites par mois", target: 15, current: 0, unit: "dossiers" },
  ...
  { id: "obj-inspections", label: "Inspections realisees", target: 10, current: 0, unit: "inspections" },
];
...
export default function ObjectivesPage() {
  ...
  useEffect(() => {
    ...
    if (saved) {
      try { setObjectives(JSON.parse(saved)); } catch { setObjectives([...DEFAULT_OBJECTIVES]); }
    } else {
    ...
    <div style={{ padding: "24px 32px", maxWidth: 800, margin: "0 auto" }}>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
        {objectives.map(obj => {
          ...
          const color = pct >= 100 ? (isInverse ? (isGood ? "#006233" : "#b42318") : "#006233") : pct >= 60 ? "#d97706" : "#b42318";
            ...
            <div key={obj.id} className="chart-card" style={{ padding: "16px 20px" }}>
              <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 10 }}>
                <input
                  ...
                  style={{ fontSize: 15, fontWeight: 700, border: "none", background: "transparent", outline: "none", flex: 1 }}
                />
                <button onClick={() => removeObjective(obj.id)} style={{
                  background: "none", border: "none", color: "var(--text-soft, #9ca3af)", cursor: "pointer", fontSize: 16,
                }}>×</button>
              </div>

              <div style={{ display: "flex", gap: 14, alignItems: "center", marginBottom: 8 }}>
                <div>
                  <label style={{ fontSize: 10, fontWeight: 600, color: "var(--text-soft)" }}>Actuel</label>
                  <input type="number" value={obj.current} onChange={e => updateCurrent(obj.id, Number(e.target.value))}
                    style={{ display: "block", width: 70, padding: "4px 8px", borderRadius: 6, border: "1px solid var(--line, #dce4ef)", fontSize: 14, fontWeight: 700 }} />
                </div>
                <div>
                  <label style={{ fontSize: 10, fontWeight: 600, color: "var(--text-soft)" }}>Objectif</label>
                  <input type="number" value={obj.target} onChange={e => updateTarget(obj.id, Number(e.target.value))}
                    style={{ display: "block", width: 70, padding: "4px 8px", borderRadius: 6, border: "1px solid var(--line, #dce4ef)", fontSize: 14, fontWeight: 700 }} />
                </div>
                <span style={{ fontSize: 12, color: "var(--text-soft)", marginTop: 14 }}>{obj.unit}</span>
                <div style={{ flex: 1 }} />
