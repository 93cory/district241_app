...
export default function PollsPage() {
  ...
  const create = async () => {
    ...
    if (!question.trim() || validOpts.length < 2) {
      showToast("Question + 2 options min.", "warning");
      return;
    }
    ...
    if (res.ok) {
      showToast("Sondage cree", "success");
      setQuestion("");
      setOptions(["", ""]);
      load();
  ...
  const vote = async (pollId: string, idx: number) => {
    ...
    if (res.ok) {
      ...
      showToast(d.detail || "Erreur", "warning");
    }
  };
      ...
      {polls.map((poll) => {
        ...
        return (
          <div key={poll.id} className="chart-card" style={{ padding: 20, marginBottom: 12 }}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
              {poll.options.map((opt: string, i: number) => {
                const pct = poll.total_votes
                  ? Math.round((poll.vote_counts[i] / poll.total_votes) * 100)
                  : 0;
                const voted = poll.my_vote === i;
                return (
                  <button
                    key={i}
                    onClick={() => poll.my_vote == null && vote(poll.id, i)}
                    ...
                    style={{
                      ...
                      padding: "8px 12px",
                      borderRadius: 10,
                      border: `1.5px solid ${voted ? "#006233" : "var(--line, #dce4ef)"}`,
                      background: voted ? "rgba(0,98,51,0.06)" : "transparent",
                      cursor: poll.my_vote == null ? "pointer" : "default",
                      textAlign: "left",
                      width: "100%",
                    }}
                  >
            ...
            <div style={{ fontSize: 11, color: "var(--text-soft, #9ca3af)", marginTop: 6 }}>
              {poll.total_votes} vote(s) · par {poll.created_by}
            </div>
          </div>
        );
      })}
