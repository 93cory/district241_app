...
export default async function CarbonPage() {
  ...
  const sectorData = sectors
    ...
    .map((s) => {
      const emData = SECTOR_EMISSIONS[s.secteur] || {
        co2_per_ati: 500,
        label: s.secteur,
        color: "#526175",
      };
    ...
    <div style={{ padding: "24px 32px", maxWidth: 900, margin: "0 auto" }}>
      ...
      <details className="pnpi-form-alert pnpi-form-alert--info" style={{ marginBottom: 20 }}>
        ...
        <p style={{ margin: "0.5rem 0 0", fontSize: 12, lineHeight: 1.5 }}>
          ...
          production effective). Pour chaque secteur, on applique un facteur d&apos;emission moyen
          exprime en kg de CO2 equivalent par operateur et par an, base sur les moyennes regionales
          CEMAC et les rapports Banque Mondiale Afrique centrale : mines 8 500 kg · BTP 4 200 ·
          chimie 3 800 · energie 2 500 · bois 1 200 · agroalimentaire 900 · peche 600. La
          compensation est calculee a raison de
      ...
      >
        <div className="chart-card" style={{ padding: "18px 16px", textAlign: "center" }}>
          ...
          <div style={{ fontSize: 24, fontWeight: 800, color: "#b42318" }}>
            {totalTonnes.toLocaleString("fr-FR")} t
          </div>
          <div style={{ fontSize: 10, fontWeight: 600, color: "var(--text-soft)" }}>
            CO2 estime (tonnes/an)
          </div>
        </div>
        <div className="chart-card" style={{ padding: "18px 16px", textAlign: "center" }}>
          <div style={{ fontSize: 28 }}>{"\uD83C\uDF32"}</div>
          <div style={{ fontSize: 24, fontWeight: 800, color: "#006233" }}>
            {compensationArbres.toLocaleString("fr-FR")}
          </div>
          <div style={{ fontSize: 10, fontWeight: 600, color: "var(--text-soft)" }}>
            ...
            <div key={s.secteur} style={{ marginBottom: 10 }}>
              ...
              >
                <span style={{ fontWeight: 600, textTransform: "capitalize" }}>
                  {s.label} ({s.count} ATIs)
                </span>
                <span style={{ fontWeight: 700, color: s.color }}>
                  {(s.co2 / 1000).toFixed(1)} tonnes
                </span>
              </div>
              <div style={{ height: 10, borderRadius: 5, background: "var(--line)" }}>
                <div
                  style={{ height: "100%", borderRadius: 5, background: s.color, width: `${pct}%` }}
                />
              </div>
            </div>
          );
      ...
      <div className="chart-card" style={{ padding: 20 }}>
        <h3 style={{ fontSize: 15, fontWeight: 700, margin: "0 0 10px", color: "#006233" }}>
          Recommandations
        </h3>
        {[
          {
            icon: "\uD83C\uDF3F",
            text: "Privilegier les certifications ISO 14001 pour le management environnemental",
          },
          {
            icon: "\u26A1",
            text: "Encourager la transition vers les energies renouvelables dans le secteur minier",
          },
          {
            icon: "\uD83C\uDF32",
            text: `Planter ${compensationArbres.toLocaleString("fr-FR")} arbres pour atteindre la neutralite carbone`,
          },
          {
            icon: "\uD83D\uDCC9",
            text: "Fixer des objectifs de reduction de 20% sur 5 ans par secteur",
          },
          {
            icon: "\uD83C\uDFED",
            text: "Inciter les operateurs a adopter des technologies propres via des avantages ATI",
          },
        ].map((r, i) => (
          <div
            key={i}
