...
export default function FeedbackPage() {
  ...
  useEffect(() => {
    fetch("/api/feedback/summary").then(r => r.ok ? r.json() : null).then(setSummary).catch(() => {});
  }, []);

  const submit = async () => {
    if (rating === 0) { showToast("Selectionnez une note", "warning"); return; }
    setSending(true);
    try {
    ...
    <div style={{ padding: "24px 32px", maxWidth: 800, margin: "0 auto" }}>
      ...
      <div className="chart-card" style={{ padding: 24, marginBottom: 20 }}>
        ...
        <textarea value={comment} onChange={e => setComment(e.target.value)}
          ...
          style={{ width: "100%", padding: "10px 14px", borderRadius: 10, border: "1.5px solid var(--line, #dce4ef)", fontSize: 13, resize: "vertical", marginBottom: 12 }} />

        <button onClick={submit} disabled={sending} style={{
          padding: "10px 24px", borderRadius: 12, border: "none",
          background: "#006233", color: "#fff", fontWeight: 700, cursor: "pointer",
          opacity: sending ? 0.6 : 1, display: "block", margin: "0 auto",
        ...
        <div className="chart-card" style={{ padding: 20 }}>
          ...
          <div style={{ display: "flex", gap: 20, alignItems: "center", marginBottom: 16 }}>
            <div style={{ textAlign: "center" }}>
              <div style={{ fontSize: 40, fontWeight: 800, color: "#f2b800" }}>{summary.average}</div>
              <div style={{ fontSize: 20, color: "#f2b800" }}>{"★".repeat(Math.round(summary.average))}</div>
            </div>
            <div style={{ flex: 1 }}>
              {[5, 4, 3, 2, 1].map(n => {
                ...
                return (
                  <div key={n} style={{ display: "flex", alignItems: "center", gap: 6, marginBottom: 3 }}>
                    ...
                    <div style={{ flex: 1, height: 8, borderRadius: 4, background: "var(--line, #e5e7eb)" }}>
                      <div style={{ height: "100%", borderRadius: 4, background: "#f2b800", width: `${pct}%` }} />
                    </div>
                    <span style={{ width: 30, fontSize: 11, textAlign: "right", color: "var(--text-soft)" }}>{count}</span>
                  </div>
                );
              })}
            </div>
              ...
              {summary.recent.slice(0, 8).map((fb: any) => (
                <div key={fb.id} style={{
                  ...
                  <div style={{ display: "flex", justifyContent: "space-between" }}>
                    <span>{"★".repeat(fb.rating)} <strong>{fb.username}</strong></span>
                    <span style={{ color: "var(--text-soft, #9ca3af)", fontSize: 11 }}>
                      {new Date(fb.created_at).toLocaleDateString("fr-FR")}
                    </span>
                  </div>
                  {fb.comment && <div style={{ marginTop: 2, color: "var(--text-soft, #526175)" }}>{fb.comment}</div>}
                </div>
              ))}
            </div>
          )}
