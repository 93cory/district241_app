...
export default function DelegationsPage() {
  ...
  const create = async () => {
    if (!to || !startDate || !endDate) {
      showToast("Champs requis", "warning");
      return;
    }
    setCreating(true);
    try {
      ...
      if (res.ok) {
        showToast("Delegation creee", "success");
        setTo("");
        setReason("");
        load();
      } else {
  ...
  const renderList = (list: Delegation[], type: "given" | "received") => (
    <div style={{ display: "flex", flexDirection: "column", gap: 6 }}>
      {list.length === 0 && (
        <p style={{ fontSize: 13, color: "var(--text-soft, #526175)" }}>Aucune delegation.</p>
      )}
      {list.map((d) => (
        <div
          key={d.id}
      ...
      <div className="chart-card" style={{ padding: 20, marginBottom: 20 }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
          <input
            ...
            placeholder="Destinataire (username)"
            style={{
              padding: "8px 12px",
              borderRadius: 8,
