    ...
    const fetchData = async () => {
      try {
        ...
        if (res.ok) {
          ...
          setKpis(data);
          setHistory(prev => {
            const next = [...prev, { time: new Date().toLocaleTimeString("fr-FR"), atis: data.atis_total || 0 }];
    ...
    <div style={{ padding: "24px 32px", maxWidth: 1000, margin: "0 auto" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16 }}>
        <div>
          ...
          <p style={{ color: "var(--text-soft)", fontSize: 13, margin: 0 }}>Rafraichissement toutes les 5 secondes</p>
        </div>
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
          <span style={{ width: 8, height: 8, borderRadius: "50%", background: "#006233", animation: "pulse 2s infinite" }} />
          <span style={{ fontSize: 12, fontWeight: 600, color: "#006233" }}>En direct</span>
          <span style={{ fontSize: 10, color: "var(--text-soft)", marginLeft: 8 }}>Tick #{tick}</span>
        </div>
      </div>

      {/* Live KPIs */}
      <div key={tick} style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(150px, 1fr))", gap: 10, marginBottom: 20 }}>
        {[
          ...
          <div key={k.label} className="chart-card" style={{ padding: "14px 16px", textAlign: "center", animation: "reveal-up 300ms ease-out" }}>
      ...
      <div className="chart-card" style={{ padding: 20 }}>
        <h3 style={{ fontSize: 14, fontWeight: 700, margin: "0 0 12px" }}>Historique temps reel (ATIs total)</h3>
        <div style={{ display: "flex", alignItems: "flex-end", gap: 3, height: 100 }}>
          {history.map((h, i) => {
            const height = (h.atis / maxAti) * 80;
            return (
              <div key={i} style={{ flex: 1, display: "flex", flexDirection: "column", alignItems: "center", gap: 1 }}>
                <div style={{ fontSize: 7, fontWeight: 700, color: "var(--text-soft)" }}>{h.atis}</div>
                <div style={{
        ...
        <div style={{ display: "flex", justifyContent: "space-between", fontSize: 9, color: "var(--text-soft, #9ca3af)", marginTop: 4 }}>
