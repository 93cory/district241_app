...
const ODD_GOALS = [
  {
    num: 8,
    title: "Travail decent et croissance economique",
    color: "#A21942",
    indicator: "emplois_estimes",
    format: (v: number) => `${v.toLocaleString("fr-FR")} emplois crees`,
  },
  {
    num: 9,
    title: "Industrie, innovation et infrastructure",
    color: "#FD6925",
    indicator: "atis_approuves",
    format: (v: number) => `${v} agrements delivres`,
  },
  {
    num: 12,
    title: "Consommation et production responsables",
    color: "#BF8B2E",
    indicator: "taux_conformite",
    format: (v: number) => `${v}% de conformite`,
  },
  {
    num: 13,
    title: "Mesures relatives au climat",
    color: "#3F7E44",
    indicator: "inspections",
    format: (v: number) => `${v} inspections environnementales`,
  },
  {
    num: 17,
    title: "Partenariats pour les objectifs",
    color: "#19486A",
    indicator: "operateurs_actifs",
    format: (v: number) => `${v} operateurs partenaires`,
  },
];
  ...
  return (
    <div style={{ padding: "24px 32px", maxWidth: 900, margin: "0 auto" }}>
            ...
            >
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 15, fontWeight: 700, marginBottom: 2 }}>
                  ODD {odd.num} · {odd.title}
                </div>
                <div style={{ fontSize: 18, fontWeight: 800, color: odd.color }}>
                  {odd.format(val)}
                </div>
              </div>
            </div>
      ...
      >
        ...
        <br />
        Agenda 2030 des Nations Unies · Republique Gabonaise
      </div>
    </div>
  );
}
