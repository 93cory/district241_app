...
export default async function InspectionDetailPage({ params }: { params: { id: string } }) {
  ...
  const canEdit =
    userRoles.some((r) => ["admin", "directeur"].includes(r)) ||
    insp.inspecteur_username === username;
  ...
  return (
    <section className="section">
      ...
      <div style={{ marginBottom: "0.75rem", fontSize: "0.875rem" }}>
        ...
        <span style={{ color: "#6b7280", margin: "0 0.5rem" }}>/</span>
        <span style={{ color: "#003F8F", fontWeight: 600, fontFamily: "monospace" }}>
          {insp.id}
        </span>
      </div>
      ...
      <div style={{ display: "flex", gap: "1.25rem", flexWrap: "wrap" }}>
        ...
        <div style={{ flex: "1 1 260px" }}>
                ...
                >
                  ...
                  >
                    {label === "Dossier ATI" && insp.ati_id ? (
                      <Link
                        href={`/pnpi/ati/${insp.ati_id}`}
                        style={{ color: "#003F8F", fontWeight: 700, textDecoration: "none" }}
                      >
                        {value}
                      </Link>
                    ) : label === "Operateur" ? (
                      <Link
                        href={`/pnpi/operateurs/${insp.operateur_id}`}
                        style={{ color: "#003F8F", fontWeight: 700, textDecoration: "none" }}
                      >
                        {value}
                      </Link>
          ...
          {insp.latitude != null && insp.longitude != null && (
            <div className="chart-card" style={{ padding: "1.25rem" }}>
              ...
              >
                <div>
                  <span style={{ color: "#6b7280" }}>Latitude :</span> {insp.latitude.toFixed(6)}°
                </div>
                <div>
                  <span style={{ color: "#6b7280" }}>Longitude :</span> {insp.longitude.toFixed(6)}°
                </div>
              </div>
              <a
        ...
        <div
          style={{ flex: "2 1 360px", display: "flex", flexDirection: "column", gap: "1.25rem" }}
        >
          {/* Observations */}
          <div className="chart-card" style={{ padding: "1.25rem" }}>
            <h3 style={{ margin: "0 0 0.75rem", color: "#003F8F", fontSize: "0.95rem" }}>
          ...
          {insp.mesures_correctives ? (
            <div className="chart-card" style={{ padding: "1.25rem" }}>
              <h3 style={{ margin: "0 0 0.75rem", color: "#92400e", fontSize: "0.95rem" }}>
            ...
            <div className="chart-card" style={{ padding: "1.25rem" }}>
              <h3 style={{ margin: "0 0 0.5rem", color: "#10b981", fontSize: "0.95rem" }}>
                Mesures correctives
              </h3>
          ...
          >
            ...
            >
              {insp.statut_conformite === "conforme"
                ? "✓"
                : insp.statut_conformite === "non_conforme"
                  ? "✗"
                  : "⚠"}
            </div>
            <div>
              ...
              <div style={{ fontSize: "0.78rem", color: "#6b7280" }}>
                {insp.statut_conformite === "conforme" &&
                  "L'operateur respecte toutes les normes applicables."}
                {insp.statut_conformite === "non_conforme" &&
                  "Des non-conformites ont ete constatees. Des mesures correctives sont requises."}
                {insp.statut_conformite === "partiel" &&
                  "Des conformites partielles ont ete constatees. Un suivi est recommande."}
              </div>
            </div>
          </div>
        </div>
