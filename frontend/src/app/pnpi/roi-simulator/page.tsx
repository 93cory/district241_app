    ...
    <div style={{ padding: "24px 32px", maxWidth: 800, margin: "0 auto" }}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 14 }}>
          <div>
            ...
            <select
              value={sector.id}
              onChange={(e) =>
                setSector(SECTORS.find((s) => s.id === e.target.value) || SECTORS[0])
              }
              style={{
                width: "100%",
                padding: "8px 12px",
          ...
          <div>
            ...
            >
              <input
                type="checkbox"
                checked={zerp}
                onChange={(e) => setZerp(e.target.checked)}
                style={{ accentColor: "#006233" }}
              />
      ...
      <div className="chart-card" style={{ padding: 20 }}>
            ...
            return (
              <div key={i} style={{ flex: 1, textAlign: "center" }}>
                <div style={{ fontSize: 8, fontWeight: 700, marginBottom: 2 }}>
                  {yearRevenu.toFixed(0)}M
                </div>
                <div
                  style={{
                    height: h,
                    background: aboveInvest ? "#006233" : "#0c7eb4",
                    borderRadius: "3px 3px 0 0",
                    minHeight: 2,
                  }}
                />
                <div style={{ fontSize: 9, color: "var(--text-soft)", marginTop: 2 }}>
                  An {i + 1}
                </div>
              </div>
            );
        ...
        <div style={{ borderTop: "2px dashed #b42318", marginTop: 4, position: "relative" }}>
          ...
          >
            Investissement: {investissement}M
          </span>
        </div>
      </div>
      ...
      >
        Simulation indicative basee sur des multiplicateurs sectoriels moyens. Les resultats reels
        dependent de nombreux facteurs.
      </div>
    </div>
  );
}
