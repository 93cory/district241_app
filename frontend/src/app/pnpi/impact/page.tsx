  ...
  return (
    <div style={{ padding: "24px 32px", maxWidth: 1000, margin: "0 auto" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(200px, 1fr))", gap: 16, marginBottom: 28 }}>
        {[
          { label: "Emplois declares", value: totalEmplois.toLocaleString("fr-FR"), color: "#006233", icon: "\u{1F477}" },
          { label: "Operateurs industriels", value: totalOps, color: "#0d3f78", icon: "\u{1F3ED}" },
          { label: "Provinces couvertes", value: `${provincesCouverts}/9`, color: "#0c7eb4", icon: "\u{1F5FA}" },
          { label: "ATI approuves", value: kpis.atis_total || 0, color: "#f2b800", icon: "\u{1F4CB}" },
          { label: "Taux conformite", value: `${kpis.taux_conformite_pct || 0}%`, color: "#006233", icon: "\u2713" },
          { label: "Indice national", value: index?.index || "\u2014", color: index?.index >= 70 ? "#006233" : index?.index >= 50 ? "#f2b800" : "#b42318", icon: "\u{1F4CA}" },
        ].map((kpi) => (
                ...
                <tr key={s.secteur}>
                  ...
                  <td style={{ textAlign: "center" }}>
                    <span style={{
                      ...
                      background: s.taux_approbation_pct >= 70 ? "#dcfce7" : s.taux_approbation_pct >= 40 ? "#fef9c3" : "#fef2f2",
                      color: s.taux_approbation_pct >= 70 ? "#166534" : s.taux_approbation_pct >= 40 ? "#854d0e" : "#b42318",
                    }}>
                      {s.taux_approbation_pct}%
                    </span>
                  </td>
                  <td style={{ textAlign: "center", fontWeight: 600 }}>{(s.emplois_declares || 0).toLocaleString("fr-FR")}</td>
                </tr>
              ))}
            </tbody>
          </table>
      ...
      {/* Province Coverage */}
      <div className="chart-card" style={{ padding: 20, marginBottom: 20 }}>
        <h2 style={{ margin: "0 0 14px", fontSize: 16, fontWeight: 700 }}>Couverture Provinciale</h2>
