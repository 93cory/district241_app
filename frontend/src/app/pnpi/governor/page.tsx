...
const PROVINCES = [
  "estuaire", "haut_ogooue", "moyen_ogooue", "ngounie", "nyanga",
  "ogooue_ivindo", "ogooue_lolo", "ogooue_maritime", "woleu_ntem",
];
    ...
    <div style={{ padding: "24px 32px", maxWidth: 900, margin: "0 auto" }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <div>
          ...
          <p style={{ color: "var(--text-soft)", fontSize: 13, margin: 0 }}>Vue dediee par province pour le gouverneur.</p>
        </div>
        <select value={province} onChange={e => setProvince(e.target.value)} style={{
          padding: "8px 14px", borderRadius: 10, border: "1.5px solid var(--line)", fontSize: 14, fontWeight: 700,
        }}>
          {PROVINCES.map(p => (
      ...
      {data && !loading && (
        <>
          {/* Score */}
          <div className="chart-card" style={{ padding: 24, textAlign: "center", marginBottom: 16 }}>
            <div style={{ fontSize: 48, fontWeight: 800, color: b.score >= 70 ? "#006233" : b.score >= 45 ? "#d97706" : "#b42318" }}>
              {b.score || 0}
            </div>
            <div style={{ fontSize: 14, fontWeight: 600, color: "var(--text-soft)" }}>Score composite provincial</div>
          </div>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(140px, 1fr))", gap: 10, marginBottom: 16 }}>
            {[
              ...
              { label: "En retard SLA", value: b.overdue || 0, color: b.overdue > 0 ? "#b42318" : "#006233" },
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16 }}>
            <div className="chart-card" style={{ padding: 20 }}>
              <h3 style={{ fontSize: 15, fontWeight: 700, margin: "0 0 10px" }}>Taux d{"'"}approbation</h3>
              <div style={{ fontSize: 36, fontWeight: 800, color: "#006233" }}>{b.approval_rate || 0}%</div>
            </div>
            <div className="chart-card" style={{ padding: 20 }}>
              <h3 style={{ fontSize: 15, fontWeight: 700, margin: "0 0 10px" }}>Taux de conformite</h3>
              <div style={{ display: "flex", gap: 12, alignItems: "baseline" }}>
                <div style={{ fontSize: 36, fontWeight: 800, color: "#006233" }}>{b.conformity_rate || 0}%</div>
                {h.non_conforme > 0 && (
                  <span style={{ padding: "3px 8px", borderRadius: 6, fontSize: 11, fontWeight: 700, background: "#fef2f2", color: "#b42318" }}>
