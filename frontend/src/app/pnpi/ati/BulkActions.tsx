...
export function BulkActions({ selectedIds, onClear }: Props) {
  ...
  const execute = async (action: "approve" | "reject") => {
    ...
    try {
      ...
      if (successCount > 0)
        showToast(
          `${successCount} ATI(s) ${action === "approve" ? "approuve(s)" : "rejete(s)"}`,
          "success",
        );
  ...
  return (
    <>
      ...
      >
        <span style={{ fontWeight: 700, fontSize: 14 }}>{selectedIds.length} selectionne(s)</span>
        <button
        ...
        >
          ...
          >
            <h3 style={{ margin: "0 0 12px", fontSize: 18, fontWeight: 700 }}>
              Rejeter {selectedIds.length} ATI(s)
            </h3>
            <textarea
              value={motif}
