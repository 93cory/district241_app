...
export function SLADashboard({ data }: { data: SLAData }) {
  const complianceColor =
    data.sla_compliance_pct >= 80
      ? "#006233"
      : data.sla_compliance_pct >= 60
        ? "#f2b800"
        : "#b42318";
  ...
  return (
    <div className="chart-card" style={{ padding: 20 }}>
      ...
      >
        ...
        >
          <div style={{ fontSize: 28, fontWeight: 800, color: complianceColor }}>
            {data.sla_compliance_pct}%
          </div>
          <div style={{ fontSize: 11, color: "var(--text-soft, #526175)", fontWeight: 600 }}>
            Conformite SLA
          </div>
        </div>
        <div
      ...
      <div style={{ display: "flex", gap: 8 }}>
        {Object.entries(data.by_priority).map(([priority, stats]) => (
          ...
          >
            ...
            <div>
              {stats.total} actif(s),{" "}
              <span style={{ color: "#b42318" }}>{stats.overdue} en retard</span>
            </div>
          </div>
        ))}
      </div>
