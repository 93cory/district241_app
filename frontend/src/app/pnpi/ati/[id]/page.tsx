  ...
  return (
    <section className="section">
      ...
      <div style={{ marginBottom: "1rem", fontSize: "0.875rem" }}>
        ...
        <Link href="/pnpi/ati" style={{ color: "#6b7280", textDecoration: "none" }}>
          ATI
        </Link>
        <span style={{ color: "#6b7280", margin: "0 0.5rem" }}>/</span>
      ...
      <div className="chart-card" style={{ padding: "1.25rem", marginBottom: "1.25rem" }}>
        ...
        >
          <div>
            ...
            >
              <h2 style={{ margin: 0, color: "#003F8F", fontFamily: "monospace" }}>
                {ati.numero_ati}
              </h2>
              <span
                style={{
            ...
            <p style={{ margin: "0.25rem 0 0", color: "#6b7280", fontSize: "0.875rem" }}>
              Operateur:{" "}
              <Link
                href={`/pnpi/operateurs/${ati.operateur_id}`}
                style={{ color: "#003F8F", fontWeight: 600, textDecoration: "none" }}
              >
                {operateurNom || ati.operateur_id}
              </Link>{" "}
              &middot; Secteur: {ati.secteur} &middot; Priorite:{" "}
            ...
            {ETAPES.map((e, i) => (
              <div key={e} style={{ flex: 1, textAlign: "center" }}>
                <div
                  style={{
                    height: "4px",
                    background: i <= etapeIdx ? "#003F8F" : "#e5e7eb",
                    marginBottom: "0.5rem",
                    borderRadius:
                      i === 0
                        ? "999px 0 0 999px"
                ...
                <span
                  style={{
                    fontSize: "0.7rem",
                    color: i <= etapeIdx ? "#003F8F" : "#9ca3af",
      ...
      <div style={{ marginBottom: "1.25rem" }}>
        <SLAClock
          dateSoumission={ati.date_soumission}
          slaJours={ati.sla_jours}
          statut={ati.statut}
        />
      </div>
      ...
      <div style={{ display: "flex", gap: "1.25rem", flexWrap: "wrap" }}>
        ...
        <div style={{ flex: "2 1 300px" }}>
          <div className="chart-card" style={{ padding: "1.25rem" }}>
            ...
            >
              {[
                ...
                [
                  "Date decision",
                  ati.date_decision
                    ? new Date(ati.date_decision).toLocaleDateString("fr-FR")
                    : "En attente",
                ],
                [
                  "Date expiration",
                  ati.date_expiration
                    ? new Date(ati.date_expiration).toLocaleDateString("fr-FR")
                    : "·",
                ],
                ...
                <div key={label as string}>
                  ...
                  <dd
                    style={{ margin: 0, fontWeight: 500, color: "#1f2937", fontSize: "0.875rem" }}
                  >
                    {value}
                  </dd>
                </div>
            ...
            {ati.statut === "rejete" && (
              <ResubmitForm atiId={ati.id} motifRejet={ati.motif_rejet} />
            )}
          </div>
          ...
          {ati.qr_code_data && (
            ...
            >
              <h3 style={{ margin: "0 0 0.75rem", color: "#10b981", fontSize: "0.95rem" }}>
                ✓ Agrement approuve · QR Code officiel
              </h3>
              <div
                style={{
                  display: "flex",
              ...
              >
                ...
                <div style={{ flex: 1 }}>
                  ...
                  <p style={{ margin: "0 0 0.75rem", fontSize: "0.75rem", color: "#6b7280" }}>
                    Ce QR code certifie l&apos;authenticite de l&apos;agrement. Il peut etre scanne
                    pour verification par les autorites competentes.
                  </p>
                ...
                {historique.map((t, i) => (
                  <div
                    ...
                    style={{
                      display: "flex",
                      gap: "0.75rem",
                      marginBottom: i < historique.length - 1 ? "1rem" : 0,
                      position: "relative",
                    }}
                  >
                    <div
                    ...
                    >
                      <div
                        style={{
                          ...
                          height: "10px",
                          borderRadius: "999px",
                          background: STATUT_COLORS[t.new_statut ?? ""] ?? "#9ca3af",
                      ...
                      {i < historique.length - 1 && (
                        <div
                          style={{ width: "1px", flex: 1, background: "#e5e7eb", marginTop: "2px" }}
                        />
                      )}
                    </div>
                    <div style={{ flex: 1, paddingBottom: "0.5rem" }}>
                      <div style={{ fontSize: "0.78rem", fontWeight: 600, color: "#1f2937" }}>
                        {t.previous_statut
                          ? `${STATUT_LABELS[t.previous_statut] ?? t.previous_statut} → `
                          : ""}
                        {STATUT_LABELS[t.new_statut ?? ""] ?? t.new_statut}
                      </div>
                      <div style={{ fontSize: "0.72rem", color: "#6b7280", marginTop: "0.1rem" }}>
                        Par {t.changed_by} &middot;{" "}
                        {new Date(t.changed_at).toLocaleDateString("fr-FR")}
                      </div>
                      {t.note && (
                        <div
                          style={{
