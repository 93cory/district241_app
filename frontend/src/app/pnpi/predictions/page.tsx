...
export default async function PredictionsPage() {
  ...
  const trendColor =
    data.approval_rate.trend === "hausse"
      ? "#006233"
      : data.approval_rate.trend === "baisse"
        ? "#b42318"
        : "#d97706";
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 16, marginBottom: 24 }}>
        ...
        <div className="chart-card" style={{ padding: 20 }}>
          ...
          <div style={{ display: "flex", gap: 20, alignItems: "baseline" }}>
            <div>
              <div style={{ fontSize: 32, fontWeight: 800, color: "#006233" }}>
                {data.approval_rate.recent_6m}%
              </div>
            ...
            <div>
              <div style={{ fontSize: 20, fontWeight: 700, color: "var(--text-soft, #9ca3af)" }}>
                {data.approval_rate.older}%
              </div>
        ...
        <div className="chart-card" style={{ padding: 20 }}>
          ...
          <div style={{ display: "flex", gap: 20 }}>
            <div>
              <div style={{ fontSize: 32, fontWeight: 800, color: "#0c7eb4" }}>
                {data.backlog.in_progress}
              </div>
              <div style={{ fontSize: 11, color: "var(--text-soft, #526175)" }}>
                Dossiers en cours
              </div>
            ...
            <div>
              <div style={{ fontSize: 20, fontWeight: 700, color: "var(--text-soft, #526175)" }}>
                {data.backlog.avg_processing_days}j
              </div>
              <div style={{ fontSize: 11, color: "var(--text-soft, #526175)" }}>Delai moyen</div>
            </div>
            <div>
              <div style={{ fontSize: 20, fontWeight: 700, color: "#d97706" }}>
                ~{data.backlog.est_clearance_days}j
              </div>
              <div style={{ fontSize: 11, color: "var(--text-soft, #526175)" }}>
                Pour tout traiter
              </div>
          ...
          <div style={{ display: "flex", gap: 14 }}>
            {data.forecasts.map((f: any) => (
              ...
              >
                ...
                <div style={{ fontSize: 28, fontWeight: 800, color: "#0c7eb4" }}>
                  {f.predicted_submissions}
                </div>
      ...
      <div className="chart-card" style={{ padding: 20 }}>
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
          {data.sector_growth.map((s: any) => {
            const growthColor =
              s.growth_pct > 0 ? "#006233" : s.growth_pct < 0 ? "#b42318" : "#526175";
            return (
              <div key={s.secteur} style={{ display: "flex", alignItems: "center", gap: 12 }}>
                <span
                  style={{ width: 100, fontSize: 13, fontWeight: 600, textTransform: "capitalize" }}
                >
                  {s.secteur}
                </span>
