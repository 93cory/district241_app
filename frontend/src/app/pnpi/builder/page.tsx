...
const WIDGET_CATALOG: {
  ...
  { type: "kpi", label: "KPI unique", endpoint: "/api/pnpi/dashboard/kpis", defaultSize: "small" },
  {
    ...
    endpoint: "/api/reports/builder?metric=atis&group_by=secteur",
    defaultSize: "medium",
  },
  {
    type: "table",
    label: "Tableau donnees",
    endpoint: "/api/reports/builder?metric=atis&group_by=mois",
    defaultSize: "large",
  },
  {
    type: "stat",
    label: "Statistique",
    endpoint: "/api/pnpi/dashboard/kpis",
    defaultSize: "small",
  },
  {
    type: "alerts",
    label: "Alertes",
    endpoint: "/api/pnpi/dashboard/smart-alerts",
    defaultSize: "medium",
  },
  {
    type: "quality",
    label: "Qualite donnees",
    endpoint: "/api/pnpi/dashboard/data-quality",
    defaultSize: "small",
  },
];
...
export default function BuilderPage() {
  ...
  const renderWidget = (w: Widget) => {
    ...
    if (!data)
      return (
        <div style={{ padding: 20, textAlign: "center", color: "var(--text-soft)", fontSize: 12 }}>
          Chargement...
        </div>
      );
    ...
    if (w.type === "kpi" || w.type === "stat") {
      ...
      return (
        <div style={{ textAlign: "center" }}>
          <div style={{ fontSize: 32, fontWeight: 800, color: "#006233" }}>{val}</div>
        </div>
      );
    }
    if (w.type === "alerts") {
    ...
    if (w.type === "quality") {
      return (
        <div style={{ textAlign: "center" }}>
          <div style={{ fontSize: 32, fontWeight: 800, color: "#006233" }}>{data.grade || "·"}</div>
          <div style={{ fontSize: 11 }}>{data.global_score}%</div>
        </div>
      );
    }
    return (
      <pre style={{ fontSize: 9, overflow: "auto" }}>
      ...
      >
        ...
        <button
          ...
          style={{
            ...
            background: editing ? "#006233" : "var(--bg-layer)",
      ...
      {widgets.length === 0 ? (
        <div className="chart-card" style={{ padding: 40, textAlign: "center" }}>
          ...
          <div style={{ fontSize: 13, color: "var(--text-soft)" }}>
            Cliquez "Modifier" pour ajouter des widgets.
          </div>
        </div>
      ) : (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3, 1fr)", gap: 12 }}>
          {widgets.map((w) => (
            <div
              key={w.id}
              style={{ gridColumn: `span ${gridCols[w.size]}`, position: "relative" }}
