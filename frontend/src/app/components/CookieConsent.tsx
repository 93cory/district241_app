    ...
    <div style={{
      ...
      <div style={{ flex: 1, minWidth: 240 }}>
        ...
        <div style={{ fontSize: 12, color: "var(--text-soft, #526175)" }}>
          La PNPI utilise des cookies techniques necessaires au fonctionnement (session, preferences).
          Aucun cookie publicitaire n'est utilise.
          <a href="/legal/confidentialite" style={{ marginLeft: 4, color: "var(--accent, #006233)", fontWeight: 600 }}>En savoir plus</a>
        </div>
      </div>
      <div style={{ display: "flex", gap: 8 }}>
        <button onClick={decline} style={{
          padding: "6px 16px", borderRadius: 8, border: "1px solid var(--line, #dce4ef)",
          background: "transparent", fontSize: 12, fontWeight: 600, cursor: "pointer",
          color: "var(--text-soft)",
        }}>Refuser</button>
        <button onClick={accept} style={{
          padding: "6px 16px", borderRadius: 8, border: "none",
          background: "#006233", color: "#fff", fontSize: 12, fontWeight: 700, cursor: "pointer",
        }}>Accepter</button>
      </div>
    </div>
  );
