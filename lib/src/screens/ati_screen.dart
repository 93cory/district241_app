...
class _ATIScreenState extends State<ATIScreen>
            ...
            children: [
              Row(
                children: [
                  ...
                  const SizedBox(width: 8),
                  _kpiBox('Approuvés', '$approuves', Colors.green.shade700),
                  const SizedBox(width: 8),
                  _kpiBox(
                    'Hors SLA',
                    '${overdueList.length}',
  ...
  Widget _buildList(List<AgrementTechniqueIndustriel> list) {
    if (list.isEmpty) {
      return Center(
        child: Column(
  ...
  Widget _buildCard(AgrementTechniqueIndustriel ati) {
    final statutColor = _statutColor(ati.statut);
    final secteurColor = _secteurColor(ati.secteur);
    final prioriteColor = _prioriteColor(ati.priorite);

    return GestureDetector(
      onTap: () => _showDetail(ati),
      child: Container(
